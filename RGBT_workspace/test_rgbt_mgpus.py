...
from lib.test.tracker.untrack import UnTrack
import lib.test.parameter.untrack as rgbt_prompt_params
...
def run_sequence(seq_name, seq_home, dataset_name, yaml_name, num_gpu=1, epoch=300, debug=0, script_name='prompt'):
    ...
    if script_name == 'untrack':
        params = rgbt_prompt_params.parameters(yaml_name, epoch)
        mmtrack = UnTrack(params)  # "GTOT" # dataset_name
        tracker = UnTrack_RGBT(tracker=mmtrack)
    ...
    for frame_idx, (rgb_path, T_path) in enumerate(zip(RGB_img_list, T_img_list)):
        ...
        if frame_idx == 0:
            ...
            modality = image[:, :, 3:]
            dummy = modality[:, :, 0:1].copy() * 0 + 2
            image = cv2.merge((image[:, :, :3], modality, dummy))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--script_name', type=str, default='untrack', help='Name of tracking method(ostrack, prompt, ftuning).')
    ...
    if dataset_name == 'GTOT':
        ...
        seq_home = '/home/zwu/Tracking/RGBT_workspace/RGBT234'
        seq_list = [f for f in os.listdir(seq_home) if isdir(join(seq_home,f))]
        seq_list.sort()
    elif dataset_name == 'LasHeR':
        seq_home = '/media/HDD2/LasHeR0428/LasHeR_Divided_TraningSet&TestingSet/TestingSet/testingset'
        seq_list = [f for f in os.listdir(seq_home) if isdir(join(seq_home,f))]
        seq_list.sort()
