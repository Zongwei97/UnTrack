...
import lib.test.parameter.untrack as rgbe_prompt_params
...
def run_sequence(seq_name, seq_home, dataset_name, yaml_name, num_gpu=1, epoch=60, debug=0, script_name='prompt'):
    ...
    if script_name == 'untrack':
        ...
        ostrack = UnTrack(params)  # "VisEvent"
        tracker = UnTrack_RGBE(tracker=ostrack)
    ...
    for frame_idx, (rgb_path, E_path) in enumerate(zip(RGB_img_list, E_img_list)):
        ...
        if frame_idx == 0:
            ...
            modality = image[:, :, 3:]
            dummy = modality[:, :, 0:1].copy() * 0 + 3
            image = np.concatenate((image[:, :, :3], modality, dummy), axis=-1)
...
if __name__ == '__main__':
    ...
    if dataset_name == 'VisEvent':
        seq_home = '/media/HDD2/VisEvent_dataset/test'
