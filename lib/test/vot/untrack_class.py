...
def run_vot_exp(tracker_name, para_name, vis=False, out_conf=False, channel_type='color'):
    ...
    print("######")
    print(para_name)
    tracker = untrack(tracker_name=tracker_name, para_name=para_name)
    print('No prob after tracker init')
    ...
    while True:
        ...
        if isinstance(imagefile, list) and len(imagefile) == 2:
            ...
            modality = image[:, :, 3:]
            if '/depth/' in imagefile[1]:
                dummy = modality[:, :, 0:1].copy() * 0 + 1 # +1 for depth
                image = cv2.merge((image[:, :, :3], modality, dummy))
            else:
                dummy = modality[:, :, 0:1].copy() * 0 + 1
                image = cv2.merge((image[:, :, :3], modality, dummy))
