...
class ToTensorAndJitter(TransformBase):
    ...
    def transform_image(self, image, brightness_factor):
        ...
        # backward compatibility
        if self.normalize:
            out = torch.cat((image[:-1, :, :].float().mul(brightness_factor/255.0).clamp(0.0, 1.0), image[-1:, :, :].float()), dim=0)
            return out
        else:
...
class Normalize(TransformBase):
    ...
    def transform_image(self, image):
        ...
        if dims[0] == 6:
            ...
            rgb = tvisf.normalize(rgb, self.mean, self.std, self.inplace)
            depth = tvisf.normalize(depth, self.mean, self.std, self.inplace)
            return torch.cat((rgb, depth, dummy), 0)

            '''
            rgb = image[:3, :, :]
            if image[3:6, :, :].max() == image[3:6, :, :].min() ==0:
                dummy = image[3:6, :, :]
                modality = image[6:, :, :]


                rgb = tvisf.normalize(rgb, self.mean, self.std, self.inplace)
                modality = tvisf.normalize(modality, self.mean, self.std, self.inplace)

                return torch.cat((rgb, dummy, modality), 0)
                ...
                return torch.cat((rgb, modality, dummy), 0)
            '''
        else:
