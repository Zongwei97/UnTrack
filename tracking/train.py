...
def parse_args():
    """
    args for training.  --script untrack --config deep_rgbx --save_dir ./output_x --mode multiple
    """
    parser = argparse.ArgumentParser(description='Parse args for training')
    # for train
    parser.add_argument('--script', type=str,  default='untrack', help='training script name')
    parser.add_argument('--config', type=str, default='deep_rgbx', help='yaml configure file name')
    parser.add_argument('--save_dir', type=str, default='./output_x', help='root directory to save checkpoints, logs, and tensorboard')
...
def main():
    ...
    #                    args.distill, args.script_teacher, args.config_teacher)
        ...
        train_cmd = "python /home/zwu/Tracking/lib/train/run_training.py " \
                    "--script %s --config %s --save_dir %s --use_lmdb %d --script_prv %s --config_prv %s --use_wandb %d " \
                    "--distill %d --script_teacher %s --config_teacher %s" \
